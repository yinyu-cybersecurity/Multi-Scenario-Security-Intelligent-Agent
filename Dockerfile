...
# 预下载 RAG Embedding 模型到正确缓存路径（~250MB多语言模型）
...
RUN python3.11 -c "from sentence_transformers import SentenceTransformer; \
    SentenceTransformer('sentence-transformers/paraphrase-multilingual-MiniLM-L6-v2')" && \
    echo "RAG model pre-downloaded (~250MB)"
