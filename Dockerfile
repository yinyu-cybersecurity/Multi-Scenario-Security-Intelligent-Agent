...
RUN set -e; \
    ...
    tar -xzf frp_0.52.3_linux_amd64.tar.gz -C /app/data/frp --strip-components=1 2>/dev/null && \
    rm -f frp_0.52.3_linux_amd64.tar.gz || true; \
    # Windows提权工具
    wget -q -O /app/data/tools/potato/PrintSpoofer64.exe \
        "https://github.com/itm4n/PrintSpoofer/releases/download/v1.0/PrintSpoofer64.exe" 2>/dev/null || true; \
    wget -q -O /app/data/tools/potato/SweetPotato.exe \
    ...
    unzip -q /tmp/mimikatz.zip -d /app/data/tools/ 2>/dev/null && \
