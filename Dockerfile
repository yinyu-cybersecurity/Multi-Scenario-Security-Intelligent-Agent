...
RUN sed -i 's@archive.ubuntu.com@mirrors.aliyun.com@g' /etc/apt/sources.list && \
    ...
    apt-get install -y --no-install-recommends \
        ...
        git curl wget unzip nmap openjdk-8-jdk maven ruby php-cli \
...
# 创建必要目录
RUN mkdir -p /app/thirdparty /app/data/sessions /app/data/tunnels /app/data/security_resources /app/data/nuclei_templates /app/.memory /app/log /opt/tools/windows /opt/frp
...
# 软链接
RUN ln -sf /usr/local/bin/fscan /opt/linux/fscan 2>/dev/null || true && \
    ln -sf /usr/local/bin/nuclei /opt/linux/nuclei 2>/dev/null || true && \
    ln -sf /usr/local/bin/xray /opt/linux/xray 2>/dev/null || true && \
    ln -sf /usr/local/bin/ffuf /opt/linux/ffuf 2>/dev/null || true && \
    ln -sf /usr/local/bin/httpx /opt/linux/httpx 2>/dev/null || true && \
    ln -sf /usr/local/bin/subfinder /opt/linux/subfinder 2>/dev/null || true && \
